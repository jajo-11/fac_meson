...
/*
 * Construct Diagonal version of Hamiltonian. This function
 * + allocates the memory for the static variable _allhams
...
int ConstructHamilton(int isym, int k0, int k, int *kg,
  ...
  int i, j, j0, t, ti, jp, jd, m1, m2, m3, ip;
  HAMILTON *h;
  SHAMILTON *hs;
  ARRAY *st;
  STATE *s;
  ...
  **/
  ...
  if (m1) {
	...
	for (t = 0; t < sym->n_states; t++) {
      ...
      j = 0;
      for (t = 0; t < k; t++) {
	h->basis[j++] = h->obs[kg[t]];
      }
      if (kp > 0 && kgp != NULL) {
	for (t = 0; t < kp; t++) {
	  h->basis[j++] = h->obs[kgp[t]];
	}
      }
    }
  }
...
#pragma omp parallel default(shared) private(i,j,t,r)
	...
	if (ip == 2 && k0 < 0) {
	  if (j < jd) {
	    ...
	    ti = kgp[j-jd]*(kgp[j-jd]+1)/2;
	  }
	}
	for (i = 0; i <= j; i++) {
	  if (i != j && i >= jd && j >= jd) r = 0;
	  else if (k0 < 0) {
	    if (ip == 1) {
	      if (i < h->odim) {
		if (j < h->odim) {
		  ...
		  r = h->oham[ot+i*(h->onbs-h->odim)+(kg[j]-h->odim)];
		}
	      } else {
		r = HamiltonElement(isym, h->basis[i], h->basis[j]);
	      }
	    } else if (ip == 2) {
	      if (i < h->ondim && j < h->ondim) {
		r = h->oham[i+t];
	      } else if (i < h->ondim) {
		r = h->oham[i+ti];
	      } else {
	...
	for (i = 0; i < h->dim; i++) {
	  ...
	  for (j = h->dim; j < h->n_basis; j++) {
		...
		r = h->oham[ot+i*(h->onbs-h->odim)+(kgp[j-h->dim]-h->odim)];
	      }
	    } else {
	      r = HamiltonElement(isym, h->basis[i], h->basis[j]);
	    }
...
int ValidBasis(STATE *s, int k, int *kg, int n) {
  return ValidBasisNK(s, k, kg, n, 0, -1, -1);
}
...
int ValidBasisNK(STATE *s, int k, int *kg, int n1, int n0, int r1, int r0) {
  ...
  if (r1 >= 0 && orb) {
    kb = GetLFromKappa(orb->kappa)/2;
    if (r0 < 0 && kb != r1) return 0;
    if (kb < r0) return 0;
    if (kb > r1) return 0;
  }
  t = -t-1;
...
int ConstructHamiltonFrozen(int isym, int k, int *kg, int n, int snc,
  ...
  for (t = 0; t < sym->n_states; t++) {
    ...
    if (ValidBasis(s, k, kg, n)) {
      if (n1 != 0 || k1 >= 0) {
	if (ValidBasisNK(s, k, kg, n1, n0, k1, k0)) {
	  jc++;
	}
      }	    
      j++;
  ...
  if (AllocHamMem(h, jc, j) == -1) goto ERROR;
  ...
  if (h->dim < h->n_basis) {
    for (t = 0; t < sym->n_states; t++) {
      s = (STATE *) ArrayGet(st, t);
      if (ValidBasis(s, k, kg, n) &&
	  ValidBasisNK(s, k, kg, n1, n0, k1, k0)) {
	h->basis[j] = t;
	j++;
      }
    }
  ...
  for (t = 0; t < sym->n_states; t++) {
    s = (STATE *) ArrayGet(st, t);
    if (ValidBasis(s, k, kg, n)) {
      if (h->dim < h->n_basis && ValidBasisNK(s, k, kg, n1, n0, k1, k0)) {
	continue;
      }    
      h->basis[j] = t;
      j++;
...
int DiagnolizeHamilton(HAMILTON *h) {
  ...
  if (m > n) {
    ...
    if (h->heff == NULL) {
      memcpy(ap, h->hamilton, sizeof(double)*h->hsize);
    } else {
...
int CompareLevels(LEVEL *lev1, LEVEL *lev2) {
  ...
  if (s1->kgroup < 0 && s2->kgroup < 0 && s1->kcfg >= 0 && s2->kcfg >= 0) {
...
int SolveStructureFrozen(char *fn, int ng, int *kg, int nc, int *kc,
  ...
  int nk, nn, k, k2, t, j, ka, i, n;
  int **bs, nts, *ts, ncs, *cs, kb, j2;
  int nlevels, jmin, jmax, j0, p0, p, m;
  ORBITAL *orb;
  SYMMETRY *sym;
  HAMILTON *h;
  STATE *s;
  ...
  nlevels = SetFrozenTargets(ng, kg, nc, kc, &nts, &ts, &ncs, &cs);
  nk = 2*k1 + 1;
  nn = n1-n0+1;
  bs = malloc(sizeof(int *)*nk);
  for (k = 0; k <= k1; k++) {
    k2 = 2*k;
    t = k2-1;
  ...
  ResetWidMPI();
#pragma omp parallel default(shared) private(k, k2, t, j, i)
  {
  for (k = 0; k <= k1; k++) {
    k2 = 2*k;
    t = k2 - 1;
...
int SolveStructure(char *fn, char *hfn,
  ...
  if (ip > 10) {
    ...
    n1 = n0%100;
    n0 = n0/100;
    int r = SolveStructureFrozen(fn, ng, kg, ngp, kgp, n0, n1, k1);
    if (ng > 0 && kg) free(kg);
...
#pragma omp parallel default(shared) private(i, h)
	...
	for (i = 0; i < ns; i++) {
	  ...
	  isp0 = malloc(sizeof(int)*h->n_basis);
	  isp1 = malloc(sizeof(int)*h->n_basis);
	  h->ohsize = h->hsize;
	  h->oham = malloc(sizeof(double)*h->ohsize);
	  h->odim = h->dim;
	  h->ondim = h->ndim;
	  h->onbs = h->n_basis;
	  h->obs = malloc(sizeof(double)*h->onbs);
	  memcpy(h->oham, h->hamilton, sizeof(double)*h->hsize);
...
int ConstructLevelName(char *name, char *sname, char *nc,
  ...
  if (basis->kgroup < 0) {
	...
	*vnl = (kl/2) + 100*n;
      ...
      if (name) {
	sprintf(av, "%d%s%c1(%d)%d",
		n, symbol, jsym, j, basis->kstate);
	sprintf(name, "%d&%s", i, av);
      }
      lev = GetLevel(i);
      si = lev->pb;
      sym = GetSymmetry(lev->pj);
	...
	if (nc) {
	  if (nele == 0) {
	    ...
	    sprintf(ashell, ".%1d*1", n);
	    strcat(nc, ashell);
	  }
	}
	if (sname) {
	  sprintf(av, "%d%s1", n, symbol);
	  if (nele == 0) {
	    sprintf(sname, "%s", av);
	  } else {
	    strcat(sname, ".");
...
int GetBasisTable(char *fn, int m0, int k0i) {
  return GetBasisTableLR(fn, m0, k0i, 0, -1);
}

int GetBasisTableLR(char *fn, int m0, int k0i, int ilev0, int ilev1) {
  FILE *f, *f1;
...
int AngularZMixStates(ANGZ_DATUM **ad, int ih1, int ih2) {
  ...
  *ad = &(angz_array[iz]);
  ...
  for (i1 = 0; i1 < ns1; i1++) {
	...
	if (n > 0) {
	  a[iz1] = malloc(sizeof(ANGZ_ARY));
	  a[iz1]->az = ang;
	  a[iz1]->nz = n;
	  if (iz2 != iz1) {
	    ang = malloc(sizeof(ANGULAR_ZMIX)*n);
	    memcpy(ang, a[iz1]->az, sizeof(ANGULAR_ZMIX)*n);
	    AngZSwapBraKet(n, ang, 0);
	    a[iz2] = malloc(sizeof(ANGZ_ARY));
	    a[iz2]->az = ang;
	    a[iz2]->nz = n;
	  }
	} else {
  ...
  (*ad)->ns = ns;
  (*ad)->nd = hams[ih1].nlevs * hams[ih2].nlevs;
  ReleaseLock(&(*ad)->lock);
  return (*ad)->ns;
}
...
#pragma omp parallel default(shared) private(i1, i2, lev1, lev2, ih1, ih2, sym1, sym2, s1, s2, ne1, ne2, ns1, ns2, ns, ad, iz, is, i, nz)
  ...
  for (i1 = 0; i1 < n1; i1++) {
	...
	if (ne1 > ne2) {
	  nz = AngularZFreeBound((ANGULAR_ZFB **)(&((ad->angz)[is]->az)),
				 is2[i2], is1[i1]);
	} else {
	  nz = AngularZFreeBound((ANGULAR_ZFB **)(&((ad->angz)[is]->az)),
				 is1[i1], is2[i2]);
...
int AngularZFreeBound(ANGULAR_ZFB **ang, int lower, int upper) {
  ...
  if (angmz_array) {
	...
	if (nz > 0) {
	  *ang = malloc(sizeof(ANGULAR_ZFB)*nz);
	  memcpy(*ang, (ad->angz)[isz0]->az, sizeof(ANGULAR_ZFB)*nz);
	}
      }
      if (nz != 0) return nz;
    }
  ...
  if (sup->kgroup < 0) {
	...
	ih2 = lev2->ibasis[j];
	isz = isz0 + ih2;
	m = (ad->angz)[isz]?(ad->angz)[isz]->nz:0;
	if (m == 1) {
	  ang_sub = (ad->angz)[isz]->az;
	  kb = ang_sub->kb;
	  r0 *= ang_sub->coeff;
	  if (fabs(r0) < angz_cut) continue;
	  ia = AddToAngularZFB(&n, &nz, ang, kb, r0);
...
int AngularZMix(ANGULAR_ZMIX **ang, int lower, int upper, int mink, int maxk,
  ...
  if (angmz_array) {
	...
	if (nz > 0) {
	  *ang = malloc(sizeof(ANGULAR_ZMIX)*nz);
	  memcpy(*ang, (ad->angz)[isz0]->az, sizeof(ANGULAR_ZMIX)*nz);
	}
      }
      if (nz != 0) {
	if (nz > 0 && ih1 > ih2) {
  ...
  if (kg1 < 0 && kg2 < 0) {
	...
	*mbk = NULL;
    ...
    for (i = 0; i < lev1->n_basis; i++) {
      ...
      ih1 = lev1->ibasis[i];
      isz0 = ih1 * hams[lev2->iham].nbasis;
      if (ih1 < hams[lev1->iham].nlevs) {
	idz0 = ih1 * hams[lev2->iham].nlevs;
      } else {
	idz0 = -1;
      }
      for (j = 0; j < lev2->n_basis; j++) {
	...
	ih2 = lev2->ibasis[j];
	isz = isz0 + ih2;
	if (idz0 >= 0 && ih2 < hams[lev2->iham].nlevs) {
	  idz = idz0 + ih2;
	} else {
	  idz = -1;
	}
	if ((ad->angz)[isz]) {
	  nz_sub = (ad->angz)[isz]->nz;
	  ang_sub = (ad->angz)[isz]->az;
	  for (m = 0; m < nz_sub; m++) {
...
double FreeANGZARY(ANGZ_ARY *az) {
  double s = 0;
  if (az == NULL) return s;
  if (az->nz > 0) {
    free(az->az);
    s = sizeof(ANGULAR_ZMIX)*az->nz;
    az->az = NULL;
    az->nz = 0;
  }
  free(az);
...
double FreeAngZDatum(ANGZ_DATUM *ap) {
  int i;
  double s = 0.0;
  for (i = 0; i < ap->ns; i++) {
    s += FreeANGZARY(ap->angz[i]);
    ap->angz[i] = NULL;
  }
  if (ap->ns > 0) {
    free(ap->angz);
    s += sizeof(ANGZ_ARY *)*ap->ns;
    if (ap->mk) {
      double *awgrid;
      int naw;
      naw = GetAWGridMBPT(&awgrid);
      for (i = 0; i < ap->nd*2; i++) {
	free(ap->mk[i]);
	s += sizeof(double)*mbpt_mk*naw;
      }
      free(ap->mk);
      s += sizeof(double *)*2*ap->nd;
    }
  }
  ap->ns = 0;
  ap->nd = 0;
...
int FreeAngZArray(int m) {
  ...
  if (angz_dim2 > 0) {
    ...
    for (i = 0; i < angz_dim2; i++) {
      ...
      s1 += FreeAngZDatum(&(angz_array[i]));
      s2 += FreeAngZDatum(&(angzxz_array[i]));
    }
    //printf("angz free size: %11.4E %11.4E\n", s1, s2);
    free(angz_array);
    free(angzxz_array);
    if (angmz_array) {
      for (i = 0; i < angz_dim2; i++) {
...
void SetOptionStructure(char *s, char *sp, int ip, double dp) {
  ...
  if (0 == strcmp(s, "structure:perturb_setzero")) {
    perturb_setzero = ip;
    return;
  }
